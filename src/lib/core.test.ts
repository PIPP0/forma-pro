...
describe('operaciones invertibles', () => {
  it('cada operación se deshace exactamente', () => {
    ...
    const ops: OpInput[] = [
      edit.block(p, 's-monto', 'b-mo-monto', 'label', 'Monto a transferir'),
      edit.addBlock(p, 's-monto', { id: 'nuevo', type: 'text', label: 'Hola' }, 1),
      edit.moveBlock(p, 's-inicio', 0, 3),
      edit.removeScreen(p, 's-ayuda'),
    ];
    for (const op of ops) {
...
describe('guardarraíl de flujo', () => {
  it('el proyecto de ejemplo no tiene errores críticos y detecta la sobrescritura suelta', () => {
    ...
    expect(issues.some((i) => i.area === 'sistema' && i.blockId === 'b-mo-continuar')).toBe(true);
  });

  it('bloquea destinos rotos y contraste insuficiente', () => {
    const p = clone(transferProject('u1'));
    p.screens.find((s) => s.id === 's-monto')!.blocks.find((b) => b.id === 'b-mo-continuar')!.target = 'no-existe';
...
describe('análisis de estudios', () => {
  it('agrega evidencia con citas por sesión', () => {
    ...
    const hes = a.themes.find((t) => t.kind === 'hesitation' && t.blockId === 'b-mo-monto')!;
    expect(hes.title).toBe('6 de 15 personas dudaron en «Monto»');
...
describe('tokens', () => {
  it('resuelve referencias por modo y calcula contraste', () => {
    const p = transferProject('u1');
    expect(resolve('{color.primary}', p.tokens, 'dark')).toBe('#7D9CFF');
  ...
  it('exporta CSS y Style Dictionary', () => {
    ...
    expect(JSON.parse(exportStyleDictionary(p.tokens)).color.dark.primary.value).toBe('#7D9CFF');
  });
